...
export async function importLeads(formData: FormData) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!rowData.lastName && !rowData.company) {
      skipped++;
      errors.push(`Row ${i + 2}: missing Last Name or Company`);
      continue;
    }

    // Default lastName to company if empty
    if (!rowData.lastName && rowData.company) {
      rowData.lastName = rowData.company;
    }
